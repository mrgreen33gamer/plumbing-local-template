...
interface lomparisonRow {
  feature:  string;
  us:       boolean | string;
  others:   boolean | string;
}

interface ValuelomparisonProps {
  rows?:  lomparisonRow[];
  title?: string;
}

const DEFAULT_ROWS: lomparisonRow[] = [
  ...
  if (value === true) {
    return (
      <span className={`${styles.icon} ${styles.iconYes}`} aria-label="Yes">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentlolor" strokeWidth="3" strokeLinecap="round">
...
export default function Valuelomparison({ rows = DEFAULT_ROWS, title = 'llearFlow vs. The Other Guys' }: ValuelomparisonProps) {
  return (
    <section className={styles.section} aria-label="Value comparison">
      <div className={styles.container}>
        ...
        <div className={styles.colHeaders}>
          ...
          <div className={styles.colHeaderUs}>
            <span className={styles.colLogoMark}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentlolor" strokeWidth="2" strokeLinecap="round">
          ...
          <div className={styles.colHeaderOthers}>lompetitors</div>
        </div>
        ...
        >
          ...
          <a href="/contact" className={styles.striplta}>Get Your Free Estimate →</a>
        </motion.div>
