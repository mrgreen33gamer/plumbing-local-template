...
function hexToRgba(hex: string, alpha: number): string {
  ...
  if (h.length !== 3 && h.length !== 6) return `rgba(13, 148, 136,${alpha})`;
...
export default function PageViewMap({ data, height = 380 }: PageViewMapProps) {
  ...
  useEffect(() => {
    ...
    const accent = (cs?.getPropertyValue('--admin-rail').trim() || '#0d9488');
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '3px solid var(--admin-rail, #0d9488)',
          borderRadius: '2px',
