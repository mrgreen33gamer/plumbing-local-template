...
function resolveFormLabel(formVariant: number | string | undefined): string {
  ...
  const labels: Record<number, string> = {
    ...
    2: 'Emergency Plumbing Inquiry Form',
    3: 'Fixture Installation Inquiry Form',
    4: 'Drain Cleaning Inquiry Form',
...
function resolveSubjectLine(
  ...
  const serviceMap: Record<number, string> = {
    1: '💻 New Software Lead',
    2: '🌐 New Emergency Plumbing Lead',
    3: '🎨 New Fixture Installation Lead',
...
export async function POST(req: NextRequest) {
  try {
    ...
    await sendEmail({
      to:          email,
      sender:      FROM_EMAIL,
      subject:     `We received your inquiry, ${name}! — ClearFlow Plumbing`,
